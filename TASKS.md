...
| T-152 PDF design improvements for Notes and Forms | Completed | Implemented shared Formify PDF export system for Forms and Notes. Uses current filled form state and current visible/canonical `notesMarkdown`; no raw transcript/audio, paid branding, or persistence. |
| T-152a Audit current Notes and Forms PDF export code | Completed | Current inline jsPDF renderers were identified in `TranscriptionClient.tsx` and `NotesClient.tsx`; both used client-side state and duplicated header/footer/page-break logic. No exportable Formify wordmark asset exists beyond the favicon. |
| T-152b Create shared PDF design utilities | Completed | Added `src/lib/pdf` with high-level `exportFormPdf` / `exportNotesPdf`, shared constants, header, metadata, wrapping, page-break, footer/page-number, date, and safe filename helpers. jsPDF remains dynamically imported inside export functions. |
| T-152c Improve Forms PDF design | Completed | Forms PDF now renders a designed printable form export: Formify header, large title, metadata panel, section heading bars, label/value rows in template field order, wrapped values, empty `—` values, footer, page numbers, and safe title-date filename. |
| T-152d Improve Notes PDF design | Completed | Notes PDF now renders current visible notes as a polished document with Formify header, title, metadata, styled H1/H2/H3, paragraphs, bullets, numbered lists, blockquote/code fallbacks, footer, page numbers, and safe title-date filename. |
| T-152e PDF ticket/docs cleanup | Completed | Updated task tracking for completed PDF work and added follow-up tickets for exportable logo assets and advanced markdown support. |
...
| T-153 Add exportable Formify wordmark asset for PDFs | Backlog | Low risk, design asset. Add a repo-owned SVG/PNG Formify wordmark suitable for embedding in generated PDFs; current PDFs use text-based branding because only favicon assets exist. |
