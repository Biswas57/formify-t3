...
| T-139 Notes sidebar and wide-desktop layout polish | Backlog | P1, low-medium risk, UI-only. Remove saved-count pill, keep Save/toggle, add smooth desktop sidebar slide, improve wide desktop notes width/spacing, and preserve mobile/narrow behaviour plus note-template CRUD. |
| T-140 Fix mobile template save button saved state | Backlog | P1, low risk, mobile UI bug. Mobile bottom save button should turn green/show saved state after successful save while preserving desktop save behaviour and template save flow. |
| T-141 Begin dark mode groundwork | Backlog | P2, medium risk, UI foundation. Track future theme strategy and token/core-surface groundwork while preserving light mode; do not implement full app-wide dark mode in one pass. |
